...
| Historical pre-news replay | **PARTIAL** | two-root production replay completed from exact upstream runs; one transformer node, 17 evidence records across eight classes, strict cutoff, fingerprinted freeze/provenance, 73.0 `evidence_backed` ranking | evaluate the frozen holdouts without retroactive reranking |
| Later-confirmation holdout | **PARTIAL** | production run `33242835298`; fail-closed plan/evidence/diagnostic schemas; four dated industrial validation windows initially `pending`; minimum source/entity diversity; Korean reader explanation; automatic reranking disabled; unfrozen security expectation gap blocked | append only evidence observed inside each frozen window; first 90-day decision boundary is 2026-11-19 |
...
| Company exposure mapping | **PARTIAL** | research-only exposure gate, cash-quality scope checks, seven-company dated comparison and Korean source/PDF renderer | product-level capacity, shipments, margins, FCF, independent market expectations; publication remains blocked |
...
falsifiers. Production later-confirmation run `33242835298` predeclares 90/180/365-day industrial
tests, starts the four required checks as `pending`, and keeps the missing pre-cutoff security
expectation snapshot blocked. The next execution gate is evidence-backed company exposure mapping
without issuing a security recommendation, while holdout evidence is appended only inside its
frozen windows. This work must
